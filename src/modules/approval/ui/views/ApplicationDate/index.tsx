import {ic_clock_circle, ic_close, ic_rp_calendar, ic_time, ic_time_application, ic_time_to, ic_timekeeping, ic_timer} from 'assets/images';
...
interface CTDatePickerProps {
  type: 'month' | 'year' | 'date' | 'time';
  value: Date;
  onValueChange: (date: Date) => void;
  headerText?: string;
}

const ApplicationDatePicker: React.FC<CTDatePickerProps> = (
  ...
  const title = useMemo(() => {
    ...
    if (type === 'time') {
      return DateUtils.format(value, DateFormatPattern.HHmm);
    }
    return '';
  }, [type, value]);
    ...
    <React.Fragment>
      ...
      <CTRbSheet height={normalize(322)} ref={pickerRef}>
        <View>
          ...
          {type === 'time' && (
            <TimePicker date={dateValue} onValueChange={d => setDateValue(d)} />
          )}
          {type === 'month' && (
            <MonthPicker
