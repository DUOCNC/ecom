...
const CheckInOut: React.FC<RequestProps> = ({
  isEditting,
  hrmData,
  requestType,
  dateFormat,
  ...
  const {reasons} = useReasonData<CheckInOutReason>(requestType);
            ...
            <RequestDetailItem key={detailItem.key}>
              <View style={style.box}>
                    ...
                    <Controller
                      ...
                      render={({field: {onChange, onBlur, value}}) => (
                        <ApplicationDatePicker
                          ...
                          value={
                            detailItem.date
                              ? new Date(detailItem.time)
                              : new Date()
                          }
                          onValueChange={v => {
                            onChange(v);
                            onSetDetailItem(detailItem.key, 'time', moment(v));
                          }}
                        />
                      )}
                    />
                ...
                <View style={style.row}>
                  <View style={style.label}>
                    <CustomerLabel text="Phạt tiền" />
                  </View>
                  <View style={style.value}>
                    <Typography
                      text={selectedReason?.hasPenance === '1' ? 'Có' : 'Không'}
                    />
                  </View>
                </View>
              </View>
