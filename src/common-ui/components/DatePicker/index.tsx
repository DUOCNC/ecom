...
const DatePicker: React.FC<CTDatePickerProps> = (props: CTDatePickerProps) => {
    ...
    <React.Fragment>
      <TouchableOpacity onPress={() => pickerRef.current?.open()}>
        <View style={CTDatePickerStyle.buttonContainer}>
          <Typography style={CTDatePickerStyle.txtValue} text={title} />
      ...
      <CTRbSheet height={normalize(322)} ref={pickerRef}>
        <View>
          ...
          {type === 'date' && (
            <DayPicker date={dateValue} onValueChange={d => setDateValue(d)} />
          )}
          {type === 'month' && (
            <MonthPicker
